...
try:
    ...
    modo_escolhido = 'SEMANAL_ATUAL_INTELIGENTE'
    # ----------------------------------------
    ...
    relatorio_colaboradores = relatorio_colaboradores.loc[(relatorio_colaboradores["Perfil de Acesso"].str.contains("LIDER EXCLUSIVO"))]
    ...
    PDOH.to_excel(
        f"outputs/BASE DE DADOS PDOH (SEM ATESTADOS E DECLARACOES) (LÍDERES) {nome_do_cliente} {primeira_data_formatada} à {ultima_data_formatada}.xlsx",
        sheet_name="Base de Dados PDOH",
        index=False,
    )

    # OTIMIZAÇÃO: Liberar memória dos dados originais que não serão mais usados
    # Nota: gerencial ainda é usado mais adiante no script (linha ~1578)
    del dados, relatorio_colaboradores, relatorio_de_checkin
    gc.collect()
    ...
    PDOH_ANALISE.loc[PDOH_ANALISE["horas_programadas"] == (pd.to_timedelta("0 days 00:00:00")),"percentual_produtividade",] = float(0)
    PDOH_ANALISE.loc[PDOH_ANALISE["horas_programadas"] == (pd.to_timedelta("0 days 00:00:00")),"percentual_visitas",] = float(0)
    PDOH_ANALISE.loc[PDOH_ANALISE["horas_programadas"] == (pd.to_timedelta("0 days 00:00:00")),"percentual_pesquisas",] = float(0)
    PDOH_ANALISE.loc[PDOH_ANALISE["horas_programadas"] == (pd.to_timedelta("0 days 00:00:00")),"percentual_efetividade",] = float(0)
    # PDOH_ANALISE.loc[PDOH_ANALISE["horas_programadas"]==(pd.to_timedelta("0 days 00:00:00")), "justificativas"] = ""

    # PDOH_ANALISE.to_excel(f"outputs/PDOH_ANALISE (LÍDERES) {nome_do_cliente} {primeira_data_formatada} à {ultima_data_formatada}.xlsx", sheet_name="PDOH_ANALISE", index=False)
    ...
    try:
        ...
        # # Nome completo da tabela de destino
        # tabela_destino = 'produtos.exclusivo_bracell_gold_relatorio_pdoh'
        
        # # Chamada da função de inserção
        # inserir_produto(PDOH_ANALISE_MYSQL, engine, tabela_destino)
    ...
    pdoh_analise_path = f"outputs/PDOH_ANALISE (SEM ATESTADOS E DECLARACOES) (LÍDERES) {nome_do_cliente} {primeira_data_formatada} à {ultima_data_formatada}.xlsx"
    PDOH_ANALISE.to_excel(pdoh_analise_path, sheet_name="PDOH_ANALISE", index=False)

    # Salvando o arquivo PDOH corretamente
    output_path = f"outputs/PDOH (SEM ATESTADOS E DECLARACOES) (LÍDERES) {nome_do_cliente} {primeira_data_formatada} à {ultima_data_formatada}.xlsx"
